...
export const login = createAsyncThunk(
    ...
    async (payload) => {
        ...
        try {
            ...
            return {
                response: error.response?.data || { msg: 'Network error occurred' },
                status: 'error',
                code: error.response?.status || 500,
            }
        }
...
export const resendVerificationLink = createAsyncThunk(
    'actions/resendVerificationLink', 
    async(payload) => {
    const {email} = payload
    try {
        const resp = await axios.post(`${URL}/api/v1/auth/resend-verification`, {
            email
        }, {
            withCredentials: true
        })
    ...
    extraReducers(builder) {
        builder
            ...
            .addCase(resendVerificationLink.pending, (state) => {
                state.verificationSendLoad = true
                state.verificationSendError = false
                state.verificationSendErrorMsg = ''
                state.verificationSendSuccessMsg = ''
            })
            ...
            .addCase(resendVerificationLink.rejected, (state) => {
                state.verificationSendLoad = false
                state.verificationSendError = true
                state.verificationSendErrorMsg = 'Unable to reset password'
            })
    }
})
