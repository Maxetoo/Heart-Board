...
export const discoverBoards = createAsyncThunk(
  ...
  {
    condition: (params = {}, { getState }) => {
      const { discoverBoards: dBoards, discoverLastEvent, discoverLastSort } = getState().board
      if (!dBoards.length) return true
      const event = params?.event ?? null
      const sort  = params?.sort  ?? 'latest'
      if (discoverLastEvent === event && discoverLastSort === sort) return false
      return true
    },
  }
)
    ...
    builder
      ...
      .addCase(discoverBoards.fulfilled, (state, action) => {
        ...
        if (status === 'success') {
          ...
          state.discoverLastSort   = action.meta.arg?.sort  ?? 'latest'
        } else {
