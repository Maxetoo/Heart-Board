...
//   {
//     id: "sponsor_200",
//     label: "Sponsor 200 curation",
//     price: 1,
//     display: "Pay $1",
//   },
//   {
//     id: "sponsor_1000",
//     label: "Sponsor 1000 curation",
//     price: 100,
//     display: "Pay $100",
//   },
//   {
//     id: "unlimited",
//     label: "Sponsor Unlimited",
//     price: 1000,
//     display: "Pay $1000",
//   },
// ];
...
//         <MicIconWrap $large={large}>
//           <BsMicFill />
//         </MicIconWrap>
//         <PlayIcon $large={large}>
...
// // ── Main modal
// const BoardViewModal = ({ board: initialBoard, onClose }) => {
...
//   const isOwner = !!(
//     currentUserId && fullBoard?.owner?._id?.toString() === currentUserId
//   );
...
//   const liked = fullBoard
//     ? likedBoardIds.includes(fullBoard._id?.toString())
//     : false;
...
//   const isMsgSender = !!(
//     currentUserId && currentMsg?.sender?._id?.toString() === currentUserId
//   );
...
//     axios
//       .get(`${URL}/api/v1/board/${board.slug}`, { withCredentials: true })
//       .then((r) => {
//         if (cancelled) return;
...
//     axios
//       .get(`${URL}/api/v1/message/${board.slug}/board`, {
...
//       if (showFlag || showSponsor || showDelete || showLogin || showFullImg)
//         closeAllModals();
//       else onClose();
//     };
...
//     if (res?.payload?.status === "success") onClose();
...
//     const onSenderClick =
//       !hideName && sender ? () => navigate(`/profile/${sender}`) : null;
...
//           ) : messages.length === 0 ? (
//             <EmptyBoard
//               onClick={() => navigate(`/board/${board.slug}/add-message`)}
//             >
...
//               {liked ? (
//                 <BsHeartFill style={{ color: "#E05A42" }} />
//               ) : (
//                 <BsHeart />
//               )}
//               <span>{likeCount}</span>
...
//                 <h3>Board Flagged</h3>
//                 <p>
//                   Thank you. We've received your report and will review this
//                   board.
//                 </p>
//                 <FlagSuccessActions>
//                   <SubmitBtn
...
//                   Why are you flagging this board? We'll review it and take
//                   action if needed.
//                 </ModalBody>
//                 <RadioGroup>
//                   {FLAG_REASONS.map((r) => (
//                     <RadioRow
//                       key={r}
//                       $selected={flagReason === r}
//                       onClick={() => setFlagReason(r)}
//                     >
//                       <RadioDot $selected={flagReason === r} />
//                       <span>{r}</span>
//                     </RadioRow>
//                   ))}
//                 </RadioGroup>
...
//           <ModalCard onClick={(e) => e.stopPropagation()}>
//             <ModalCaption>
//               Board space max out {fullBoard?.stats?.messages ?? 0}/1000
//             </ModalCaption>
...
//                 <RadioRow
//                   key={opt.id}
//                   $selected={sponsorChoice === opt.id}
//                   onClick={() => setSponsorChoice(opt.id)}
//                 >
//                   <RadioDot
//                     $selected={sponsorChoice === opt.id}
//                     $green={sponsorChoice === opt.id}
//                   />
...
//               ))}
//             </RadioGroup>
//             <SubmitBtn onClick={() => setShowSponsor(false)}>
//               Continue
//             </SubmitBtn>
//           </ModalCard>
//         </ModalOverlay>
...
//             {fullBoard?.owner?.username && (
//               <ShareBoardOwner>by {fullBoard.owner.username}</ShareBoardOwner>
//             )}
//             <ShareLinkRow>
...
//               <strong>{fullBoard?.title}</strong>? This cannot be undone.
//             </ModalBody>
//             <DeleteRow>
...
//             </ModalBody>
//             <DeleteRow>
//               <CancelBtn onClick={() => setShowDeleteMsg(false)}>
//                 Cancel
//               </CancelBtn>
...
//                 onSenderClick={
//                   (!isAnonymous || isMsgSender) && currentMsg.sender?.username
//                     ? () => navigate(`/profile/${currentMsg.sender.username}`)
//                     : null
//                 }
//                 large
//               />
...
//             <ActionsMenuPortal
//               style={{ top: msgMenuPos.top, right: msgMenuPos.right }}
//             >
//               <ActionsMenuItem
//                 onClick={() => {
//                   setShowMsgMenu(false);
//                   navigate(`/message/${currentMsg._id}/edit`);
//                 }}
//               >
//                 <BsPencil />
...
//                 onClick={() => {
//                   setShowActions(false);
//                   navigate(`/board/${board.slug}/edit`);
//                 }}
//               >
//                 <BsPencil />
...
// const EmptyBoardText = styled.p`
//   font-size: 0.88em;
//   color: rgba(255, 255, 255, 0.5);
//   margin: 0;
//   text-align: center;
// `;
// const EmptyBoardCta = styled.p`
//   font-size: 0.78em;
//   color: #e05a42;
//   margin: 0;
//   font-weight: 600;
...
//   ${({ $clickable }) =>
//     $clickable && `&:hover { background: rgba(0,0,0,0.75); }`}
// `;
...
//   }
//   &:hover {
//     color: ${({ $danger }) => ($danger ? "#ff6b55" : "#fff")};
//   }
// `;
...
//   position: relative;
// `;
// const ActionsMenuBackdrop = styled.div`
//   position: fixed;
//   inset: 0;
//   z-index: 500;
// `;
...
//   border: 2px solid
//     ${({ $selected, $green }) =>
//       $selected ? ($green ? "#22c55e" : "#E05A42") : "#ccc"};
//   background: ${({ $selected, $green }) =>
//     $selected ? ($green ? "#22c55e" : "#E05A42") : "transparent"};
...
//   margin-bottom: 12px;
// `;
// const ShareBoardName = styled.h4`
//   font-size: 0.95em;
//   font-weight: 700;
//   color: #111;
//   margin: 0 0 2px;
// `;
// const ShareBoardOwner = styled.p`
//   font-size: 0.8em;
//   color: #888;
//   margin: 0 0 16px;
// `;
...
//   border-radius: 8px;
// `;
// const FullCanvasWrap = styled.div`
...
//   background: rgba(201, 79, 56, 0.15);
//   color: #c94f38;
//   font-size: 0.67em;
//   font-weight: 600;
//   padding: 3px 9px;
...
//   pointer-events: ${({ $clickable }) => ($clickable ? "auto" : "none")};
//   ${({ $clickable }) =>
//     $clickable && `&:hover { background: rgba(201,79,56,0.28); }`}
// `;
      ...
      <Panel>
        ...
        <Meta>
          <ActionsRow>
            ...
            {isLoggedIn && (
              <ActionBtn $accent onClick={() => navigate(`/board/${board.slug}/add-message`)}>
                <BsPlusCircle />
                <span>Add Post</span>
              </ActionBtn>
            )}
...
const MediaArea = styled.div`
  position: relative;
  width: calc(100% - 48px); margin: 6px auto 0;
  aspect-ratio: 1 / 1; background: #111420;
  border-radius: 14px; overflow: hidden; flex-shrink: 0;
  @media (max-width: 600px) { width: calc(100% - 24px); }
`
...
const Meta = styled.div`
  flex-shrink: 0; padding: 14px 18px 28px; margin-top: 6px; color: #fff; overflow-y: auto;
`
...
const BoardTitle = styled.h2`font-size: 1.05em; font-weight: 700; color: #fff; margin: 0 0 2px;`
const BoardTag   = styled.p`font-size: 0.78em; color: #9CA3AF; margin: 0 0 4px;`
...
const FullAudioWrap  = styled.div`
  width: min(92vw, 420px); aspect-ratio: 1/1; background: #F0E0DC;
...
const MicBtn = styled.button`
  ...
  display: flex; align-items: center; justify-content: center;
  cursor: pointer; animation: ${pulse} 2.4s ease-in-out infinite; transition: transform 0.15s;
...
`
const AudioBottom = styled.div`
  width: ${({ $large }) => $large ? '78%' : '72%'}; display: flex; flex-direction: column; gap: 5px;
`
const AudioTrack = styled.div`
