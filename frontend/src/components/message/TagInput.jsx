...
const TagInput = ({ onMentionChange, onTagsChange, initialMention }) => {
  ...
  const [tags, setTags]                   = useState(() => {
    if (initialMention) {
      if (initialMention.startsWith('#')) {
        return [{ id: Date.now(), value: initialMention, type: 'hashtag' }]
      }
      const username = initialMention.replace(/^@/, '')
      return [{ id: Date.now(), value: `@${username}`, type: 'user' }]
    }
    return []
  })
  ...
  const [mentionStatus, setMentionStatus] = useState(initialMention ? 'found' : null)

  // Pre-populate receipentInputChange so the preview button enables
  useEffect(() => {
    if (initialMention) {
      if (initialMention.startsWith('#')) {
        onMentionChange?.(initialMention)
        return
      }
      const username = initialMention.replace(/^@/, '')
      dispatch(receipentInputChange(`@${username}`))
  ...
  const hasHashtag = tags.some(t => t.type === 'hashtag')

  // Notify parent of @mention or #hashtag change
  useEffect(() => {
    const mentionTag  = tags.find(t => t.type === 'user')
    const hashtagTag  = tags.find(t => t.type === 'hashtag')
    onMentionChange?.(mentionTag?.value ?? hashtagTag?.value ?? null)
  ...
  const handleChange = (e) => {
    ...
    if (last.startsWith('@') && last.length > 1) {
      ...
      const q = last.slice(1).toLowerCase()
      // If no receipent set yet, treat as hashtag receipent first
      if (!hasMention && !hasHashtag) {
        // Hashtag receipent — no API check needed, just track it
        setSuggestions([])
        setSuggestType('hashtag')
        setMentionQuery('')
      } else {
  ...
  const commitInput = useCallback((value) => {
    ...
    if (value.startsWith('@')) {
      ...
      // First #tag with no receipent yet = hashtag receipent; otherwise event
      type = (!hasMention && !hasHashtag) ? 'hashtag' : 'event'
    } else {
      type = 'plain'
    }
    if (type === 'user' && hasMention) return
    if (type === 'hashtag' && hasHashtag) return
    if (type === 'user' && mentionStatus !== 'found') return
    // hashtag receipent: no validation needed, always allowed
    ...
    setSuggestType(null)
    setMentionQuery('')
  }, [hasMention, hasHashtag, mentionStatus, tags])
  ...
  const placeholder = tags.length === 0
    ? 'Send to: @username or #hashtag · #event tags'
    : (hasMention || hasHashtag) ? 'Add #event tags...' : 'Add #event tags or @username'
    ...
    <TagInputWrapper ref={wrapperRef}>
      ...
      {tags.length > 0 && (
        <div className="tag_hint">
          {(hasMention || hasHashtag)
            ? <span>✓ Recipient set · Add more <strong>#event</strong> tags</span>
            : <span>Type <strong>@username</strong> or <strong>#hashtag</strong> to set recipient</span>
          }
        </div>
      )}
