...
const CreateMessagePage = () => {
  ...
  const handlePost = useCallback(async () => {
    ...
    dispatch, slug, hasContent, pendingCanvasFile, pendingAudioFile,
    canvasText, canvasBg, canvasFrame, canvasVector,
  ])

  const closePreview = () => {
    setShowPreview(false); setPostError('')
    ...
    return (
      <Wrapper>
        <SuccessCard>
          ...
          <button className="done_btn" onClick={() => navigate(`/board/${slug}`)}>
            View Board
          </button>
        </SuccessCard>
      </Wrapper>
  ...
  return (
    <AudioThumb>
      ...
      <div className="progress_track">
        <div className="progress_fill" style={{ width: duration ? `${(currentTime / duration) * 100}%` : '0%' }} />
      </div>
    </AudioThumb>
  )
}
...
const AudioThumb = styled.div`
  ...
  .play_btn {
    ...
    background: var(--primary-color, #EF5A42); color: #fff;
    display: flex; align-items: center; justify-content: center;
    font-size: 1.8em; cursor: pointer; padding-left: 4px;
    transition: transform 0.15s; &:hover { transform: scale(1.06); }
    svg { display: block; }
  }
...
const SuccessCard = styled.div`
  ...
  .done_btn {
    ...
    background: var(--primary-color, #EF5A42); color: #fff;
    font-size: 1em; font-weight: 600; cursor: pointer; transition: opacity 0.2s;
    &:hover { opacity: 0.88; }
  }
`
